					...
					$.ajax({
						...
						'success': function (data) {
							...
							//	data = JSON.parse(data);
								...
								$('#status-row div#status').html('<span class = "error">return type is ' + typeof(data) + '</span>');
							}
						}
					})
				})
			...
			'output_campaigns_cc': function (data) {
				...
				if (data.campaigns != undefined ) {
					...
					$.each(data.campaigns, function (index, val) {
						console.log(this);
						console.log(admin.utils.lookup_campaign_rec(admin.data.db_campaigns, this.campaign_id));
						if (admin.utils.lookup_campaign_rec(admin.data.db_campaigns, this.campaign_id) == undefined) {
						   ...
						   if (this.current_status=='Done') {
    							...
    							if (this.current_status=='Done') {
									...
									console.log((data.campaigns.length -1) + '::::' + index)
						...
						if ((data.campaigns.length -1) == index) {
							$('#cc-campaigns #loader-wrap').addClass('hidden');
						}
					})
